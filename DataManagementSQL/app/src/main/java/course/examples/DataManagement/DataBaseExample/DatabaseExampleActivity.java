...
public class DatabaseExampleActivity extends ListActivity {
	...
	private void insertAuthors() {
		...
		values.put(DatabaseOpenHelper.AUTHOR_NAME, "Frank Sinatra");
		mDbHelper.getWritableDatabase().insert(DatabaseOpenHelper.TABLE_NAME, null, values);
        //databases don't work well as readable and writable at the same time


		values.clear();

		values.put(DatabaseOpenHelper.AUTHOR_NAME, "Lady Gaga");
		mDbHelper.getWritableDatabase().insert(DatabaseOpenHelper.TABLE_NAME, null, values);

		values.clear();

		values.put(DatabaseOpenHelper.AUTHOR_NAME, "Jawny Cash");
		mDbHelper.getWritableDatabase().insert(DatabaseOpenHelper.TABLE_NAME, null, values);

		values.clear();

		values.put(DatabaseOpenHelper.AUTHOR_NAME, "Ludwig van Beethoven");
		mDbHelper.getWritableDatabase().insert(DatabaseOpenHelper.TABLE_NAME, null, values);
	}
